	...
	class Model extends CI_Model {
        ...
        function sent_survei(){
           ...
           foreach($data as $val){
               ...
               $this->db->group_by("hp");
               $dt = $this->db->get("data_order")->result();
               $n=0;
               foreach($dt as $v){ 
                   $n++;
        ...
        //         return $var;
            ...
            $mimetype    = $this->input->post("mimetype");
            $fileData    = $this->input->post("file");
            $to          = "6285794151322";//$this->input->post("to");
            ...
            if(!$open){
                ...
                if(!$info_tutup){
                     $info_tutup           = "Mohon maaf hari ini kami tidak buka 🙂";
                }else{
                 $info_tutup = str_replace("\n","\\n",$info_tutup);
                 $info_tutup = str_replace("\r","\\n",$info_tutup);
                }
                
                 $var["replay"] = '{"title":"Hai kak '.$nama.'!!","body":"'.$info_tutup.'","footer":"'.$client->sender_name.'"}';
                 $var["jenis_pesan"] = 4;
            ...
            if($time<$open || $time>$close){
                 ...
                 $info_tutup = "Jam operasional kami hari ini pukul ".$open." sampai dengan pukul ".$close." WIB 😊";
                 $var["replay"] = '{"title":"Hai kak '.$nama.'!!","body":"'.$info_tutup.'","footer":"'.$client->sender_name.'"}';
                 $var["jenis_pesan"] = 4;
                 $var["options"] = '[{"id":"ig","text":"Instagram","url":"'.$client->ig.'"},{"id":"info","text":"🤩 Info menarik"},{"id":"story","text":"🤠 Story '.$client->sender_name.'"}]';
                 $var["status"] = true;return $var;
            ...
            if($btn=="sudah_sesuai"){
              ...
              if($btn=="order_ulang"){
                ...
                if(isset($db->id)){
                 ...
                 $var["replay"] = '{"title":"Hai kak '.$nama.' 👋\nSelamat datang di 📍'.$client->sender_name.' !!","body":"'.$sambutan.'","footer":"'.$client->sender_name.'"}';
                 $var["img"] = isset($client->foto_menu)?(base_url()."file_upload/".$this->sender($to)."/".$client->foto_menu):null;
                 $var["jenis_pesan"] = 4;
                 $this->simpan_replay();
