...
class Wa_broadcast extends MY_Controller {
	...
	function getData()
	{
		...
		foreach ($list as $dataDB) {
			...
			if($dataDB->jenis==0){
				$jenis = "<span class='badge badge-success'>Whatsapp</span>";
				$tombol = '<div class="text-right"> 
				...
				$jenis = "<span class='badge badge-primary'>Kontak</span>";
	...
	function sendBroadcast_group(){
		...
		$dt = $this->mdl->sendBroadcast_group();
		$data["data"]=$dt;
		$data["token"] = $this->m_reff->getToken();
		echo json_encode($data);
	}
	...
	function sendBroadcast_group_kontak(){
		...
		$dt = $this->mdl->sendBroadcast_group_kontak();
		$data["data"]=$dt;
		$data["token"] = $this->m_reff->getToken();
		echo json_encode($data);
	}
