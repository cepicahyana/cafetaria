...
class Model_kontak_group extends CI_Model  {
	...
	function getById($id)
	{
		return $this->db->get_where('data_kontak', ['id'=>$id]);
	}
	function get_data()
	{
		$this->_get_data();
	...
	function _get_data()
	{
		...
		if (strlen(isset($_POST['search']['value'])?($_POST['search']['value']):null)>1) {
			...
			$query=$this->db->from("data_kontak");
		return $query;
	}
	...
	{
		return $this->db->get_where('data_kontak', ['id_group'=>$id])->num_rows();
	}
...
// 		$this->m_reff->log("delete kontak group");
		...
		$this->db->where("id",$id);
		return    $this->db->delete("data_kontak");
	}
