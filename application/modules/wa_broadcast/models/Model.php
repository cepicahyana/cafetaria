...
class Model extends CI_Model  {
	...
	function getById($id)
	{
		return $this->db->get_where('data_group', ['id'=>$id]);
	}
...
// 		$this->m_reff->log("Delete broadcast group id : ".$id);
		...
		$countKontak = $this->db->get_where('data_kontak', ['id_group'=>$id])->num_rows();
		if ($countKontak > 0) {
...
// 			$this->m_reff->log("Delete group broadcast");
		...
		$this->db->where("id",$id);
		return $this->db->delete("data_group");;
	}
	...
	function senddata_group_kontak(){
		$form	=	$this->input->post("f");
		$file	 =	$this->input->post("lampiran"); 
		$this->db->set("file",$file);
