...
<input type="hidden" id="formToken" name="<?php echo $this->m_reff->tokenName()?>" value="<?php echo $this->m_reff->getToken()?>">
  ...
  <div class="col-md-12 col-lg-12 col-xl-12 mx-auto d-block" data-select2-id="13" id="loading">
									  ...
									  <h6 class="card-titles ">  No NIK Pegawai:</h6>
...
<div id="form_akun">
                                      ...
                                      <input type="text" autocomplete="off" onkeyup="getDataByNik(this.value)" name="f[nik]"  class='form-control'>
								        </div> 
 ...
 <div class="row  ">
 <?php
 $jk_peg   = $this->m_reff->dataProfilePegawai()->jk;
 if($jk_peg=="l"){
    $this->db->select("id,jk,nama_p as nama");
}else{
    $this->db->select("id,jk,nama_l as nama");
}
$data = $this->db->get("tr_hubungan")->result();
foreach($data as $val){
...
echo '<div class="col-lg-3 p-1"> <label class="rdiobox">
<input onclick="klikhub(`'.strtolower($val->nama).'`,`'.$jk.'`)" required type="radio" name="id_hubungan" value="'.$val->id.'"><span>'.$val->nama.'</span></label> </div>';
}?>
</div>
