		...
		<form  action="javascript:submitForm('modal_edit')" id="modal_edit" url="<?php echo base_url()?>autoreplay/update_autoreplay"  method="post" enctype="multipart/form-data">
			...
			<div class=" pd-sm-20 "  >
				...
				<!--</div>-->
				    ...
				    {
				          $title = $val['body'];
				          $desc = isset($val['url'])?($val['url']):null;
				          if(!$desc){
				              $desc = isset($val['number'])?($val['number']):null; 
				          }
				        echo '<div class="col-md-12 mb-2 row">
					    <div class="col-md-2">Tombol ke '.$i.' :</div>
					    <div class="col-md-4"><input type="text" name="btn[]" class="form-control" value="'.$title.'"></div>
