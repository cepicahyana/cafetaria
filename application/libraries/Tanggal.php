...
class tanggal
{
         ...
         function selisih_waktu($waktu_awal,$wakti_akhir=null){
             if(!$wakti_akhir){
                 $wakti_akhir = date("Y-m-d H:i:s");
             }
        	     $awal  = new DateTime($waktu_awal);
            $akhir = new DateTime($wakti_akhir); // Waktu sekarang
            $diff  = $awal->diff($akhir);
            
          $y = ""; $m = ""; $d = ""; $h = ""; $i = ""; $s = "";
           if($diff->y){ return $y = ''.$diff->y.'  tahun '; }
           if($diff->m){ return $m = ''.$diff->m.' bulan ';}
           if($diff->d){ return $d = ''.$diff->d.' hari ';}
           if($diff->h){ return $h = ''.$diff->h.' jam ';}
           if($diff->i){ return $i = ''.$diff->i.' menit ';}
           if($diff->s){ return $s = ''.$diff->s.' detik ';}
           return $y.$m.$d.$h.$i.$s;
         }
	function ind($id,$di)
	{
